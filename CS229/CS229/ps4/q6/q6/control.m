...
min_attempts_to_start_display = 400;
display_started=0;
...
% while (number of consecutive no learning trials < NO_LEARNING_THRESHOLD)
  ...
  % model.
  for a = 1:NUM_ACTIONS
      chooser(a)=transitionprob(state,:,a)*values;
  end
  [~,action] = max(chooser);
...
%   R=R-abs(x)-abs(theta)*10-abs(theta_dot);
  ...
  transitionprobest(state,new_state,action)=transitionprobest(state,new_state,action)+1;
  rewardsest(state) = rewardsest(state) + R;
  ...
  if (new_state==NUM_STATES)
    ...
    for a=1:NUM_ACTIONS
        transitionprob(:,:,a)=transitionprobest(:,:,a)./sum(transitionprobest(:,:,a),2);
    end
    rewards = rewardsest./statecount;
    rewards(statecount==0) = 0;
    transitionprob(isnan(transitionprob))=1/NUM_STATES;
    ...
    while(true)
        ...
        utility = zeros(NUM_STATES,NUM_ACTIONS);
        for a = 1:NUM_ACTIONS
          utility(:,a)=transitionprob(:,:,a)*values;
        end
        [~,bestaction] = max(utility,[],2);
        
        newvalues=zeros(NUM_STATES,1);
        for s = 1:NUM_STATES
            newvalues(s) = rewards(s) + GAMMA*transitionprob(s,:,bestaction(s))*values;
        end
        if max(abs(values-newvalues))<TOLERANCE
            values = newvalues;
            if iter==1
